...
# Data Source
########################################################
module "datasource_bda" {
  source = "../modules/datasource"

  prefix = "${local.prefix}-${local.region}-bda"
  datasource = {
    force_destroy = var.datasource.force_destroy
  }
}
...
resource "aws_iam_policy" "knowledge_bases" {
  ...
  */
  policy = templatefile("./policy/iam_kb.json", {
    datasource_bucket_arn = module.datasource.bucket.arn
    output_bucket_arn     = module.datasource_bda.bucket.arn
    account_id            = local.account_id
...
#         model_arn = local.claude3_sonnet_model_arn
#       }
#     }
#   }
# }
