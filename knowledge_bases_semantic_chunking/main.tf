...
resource "aws_s3_object" "momotarou_chapter1_no_period" {
  # アップロード元(ローカル)
  source = "../サンプルドキュメント/semantic-chunking/桃太郎第1章_句点なし.txt"
  # アップロード先(S3)
  key    = "桃太郎第1章_句点なし.txt"
  bucket = module.datasource.bucket.id

  # エンティティタグ (ファイル更新のトリガーに必要)
  source_hash = filemd5("../サンプルドキュメント/semantic-chunking/桃太郎第1章_句点なし.txt")
}
